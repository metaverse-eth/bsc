...
contract METAVERSE is ERC20, Ownable {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "METAVERSE: The dividend tracker already has that address");

        METAVERSEDividendTracker newDividendTracker = METAVERSEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "METAVERSE: The new dividend tracker must be owned by the METAVERSE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "METAVERSE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "METAVERSE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "METAVERSE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "METAVERSE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "METAVERSE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "METAVERSE: Cannot update gasForProcessing to same value");
...
contract METAVERSEDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("METAVERSE_Dividen_Tracker", "METAVERSE_Dividend_Tracker") {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "METAVERSE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "METAVERSE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main METAVERSE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "METAVERSE_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours");
        require(newClaimWait != claimWait, "METAVERSE_Dividend_Tracker: Cannot update claimWait to same value");
